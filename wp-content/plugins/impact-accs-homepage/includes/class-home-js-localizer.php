...
class IAH_Home_Js_Localizer {
	...
	public static function map() {
		...
		$exact = IAH_DIR . 'includes/i18n/ru-home-exact.php';
		if ( is_readable( $exact ) ) {
			$labels = require $exact;
			if ( is_array( $labels ) ) {
				self::$map = array_merge( self::$map, $labels );
			}
		}
	...
	public static function localize_js( $js ) {
		...
		$exact_keys = array();
		$exact_file = IAH_DIR . 'includes/i18n/ru-home-exact.php';
		if ( is_readable( $exact_file ) ) {
			$exact_map = require $exact_file;
			if ( is_array( $exact_map ) ) {
				$exact_keys = array_fill_keys( array_keys( $exact_map ), true );
			}
		}
		...
		foreach ( $map as $en => $ru ) {
			...
			$is_exact = isset( $exact_keys[ $en ] );
			if ( strlen( $en ) < 12 && ! $is_exact ) {
	...
	/**
	 * Homepage chunks that must be served via iah-home-js (hero UI + header sound).
	 *
	 * @return array<int,string>
	 */
	public static function localized_chunk_names() {
		// No chunk intercept on homepage — any RU bundle in the hydration tree causes React #418 / black 3D.
		return array();
	}

	/**
	 * Intercept selected homepage chunk loads for RU (avoids full-bundle intercept).
	 *
	 * @return string
	 */
	public static function hero_intercept_script() {
		return self::chunks_intercept_script( self::localized_chunk_names() );
	}

	/**
	 * Intercept listed chunk filenames for RU homepage.
	 *
	 * @param array<int,string> $chunks Chunk basenames.
	 * @return string
	 */
	public static function chunks_intercept_script( $chunks ) {
		if ( ! self::is_ru() || empty( $chunks ) ) {
			return '';
		}

		$local = esc_js( trailingslashit( home_url( 'iah-home-js' ) ) );
		$names = wp_json_encode( array_values( $chunks ) );
		if ( ! is_string( $names ) ) {
			return '';
		}

		return '<script>(function(){var L="' . $local . '",N=' . $names . ';function hit(u){if(typeof u!=="string")return"";for(var i=0;i<N.length;i++){if(u.indexOf(N[i])!==-1)return N[i]}return""}function rw(u){var h=hit(u);if(!h)return u;var p=u;try{if(u.indexOf("http")===0){var o=location.origin;if(u.indexOf(o)===0)p=u.slice(o.length);else return u}}catch(e){}return L+h}var f=window.fetch;if(f)window.fetch=function(i,n){if(typeof i==="string")i=rw(i);else if(i&&i.url)i=new Request(rw(i.url),i);return f.call(this,i,n)};var X=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(){arguments[1]=rw(arguments[1]);return X.apply(this,arguments)};var ps=Object.getOwnPropertyDescriptor(HTMLScriptElement.prototype,"src");if(ps&&ps.set)Object.defineProperty(HTMLScriptElement.prototype,"src",{set:function(v){ps.set.call(this,rw(v))},get:ps.get})})();</script>';
	}
