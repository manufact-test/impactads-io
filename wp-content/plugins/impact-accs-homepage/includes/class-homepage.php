	...
	public static function render_document() {
		...
		if ( $iac_url && class_exists( 'IAC_Feature_Page' ) ) {
			...
			if ( 'ru' === $lang ) {
				...
				$inline_css .= 'header nav a[href$="/about"],header nav a[href$="/about/"]{font-size:0!important}header nav a[href$="/about"]::before,header nav a[href$="/about/"]::before{content:"О НАС";font-size:.875rem}header nav a[href$="/blog"],header nav a[href$="/blog/"]{font-size:0!important}header nav a[href$="/blog"]::before,header nav a[href$="/blog/"]::before{content:"БЛОГ";font-size:.875rem}header nav a[href*="/contact"]{font-size:0!important}header nav a[href*="/contact"]::before{content:"КОНТАКТЫ";font-size:.875rem}header button[aria-haspopup="true"]{font-size:0!important}header button[aria-haspopup="true"]::before{content:"АККАУНТЫ";font-size:.875rem}header button[data-slot="button"]{font-size:0!important}header button[data-slot="button"]::after{content:"ПОДОБРАТЬ АККАУНТЫ";font-size:.875rem}header .lg\\:hidden>button{font-size:0!important}header .lg\\:hidden>button::after{content:"МЕНЮ";font-size:.9375rem}header button[aria-label="Включить звук"] span.font-misc,header button[aria-label="Enable sound"] span.font-misc,header button[aria-label="Unmute sound"] span.font-misc,header button[aria-label="Выключить звук"] span.font-misc,header button[aria-label="Mute sound"] span.font-misc{font-size:0!important}header button[aria-label="Включить звук"] span.font-misc::after,header button[aria-label="Enable sound"] span.font-misc::after,header button[aria-label="Unmute sound"] span.font-misc::after{content:"ЗВУК: ВЫКЛ";font-size:.875rem}header button[aria-label="Выключить звук"] span.font-misc::after,header button[aria-label="Mute sound"] span.font-misc::after{content:"ЗВУК: ВКЛ";font-size:.875rem}';
			}
			$head_css  = '<link rel="stylesheet" href="' . $wp_css . '" />'
				. '<link rel="stylesheet" href="' . $tools_css . '" />'
				. '<link rel="stylesheet" href="' . $menu_css . '" />'
				. '<style id="iah-home-critical-css">' . $inline_css . '</style>';
		...
		$inline .= self::asset_rewrite_script( $prefix, $lang );
		...
		if ( $iac_url ) {
			...
			$hero_js   = esc_url( IAH_URL . 'assets/js/hero-copy-ru.js?v=' . rawurlencode( IAH_VERSION ) );
			...
			if ( 'ru' === $lang ) {
				$inject .= '<script src="' . $hero_js . '"></script>';
			}
